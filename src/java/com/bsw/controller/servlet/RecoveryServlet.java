...
public class RecoveryServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
      ...
      //  processRequest(request, response);
        ...
        HashMap map = TbBwCustController.getInstance().updatePasswordByUsername(username,password);
        PrintWriter out = response.getWriter();
        try {
            /* TODO output your page here. You may use following sample code. */
            out.println("<!DOCTYPE html>");
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Recovery</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Recovery Password " + map.get("status") + "</h1>");
            out.println("</body>");
            out.println("</html>");
        } finally {
            out.close();
